...
In the `.cpp` file are going to put all the logic inside the `Tick` function to check if we hit anything. First, make sure that it is still valid to move. Add the below `if` statement to the `Tick` function.
